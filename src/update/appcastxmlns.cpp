#include "appcastxmlns.h"

QTX_BEGIN_NAMESPACE

namespace __Appcast {

const char *kRssTitleXmlName                    = "title";
const char *kRssLinkXmlName                     = "link";
const char *kRssItemXmlName                     = "item";
const char *kRssEnclosureXmlName                = "enclosure";
const char *kRssEnclosureUrlXmlAttr             = "url";
const char *kRssEnclosureTypeXmlAttr            = "type";

const char *kSparkleXmlNamespaceUri             = "http://www.andymatuschak.org/xml-namespaces/sparkle";
const char *kSparkleMinimumSystemVersionXmlName = "minimumSystemVersion";
const char *kSparkleEnclosureVersionXmlAttr     = "version";

}

QTX_END_NAMESPACE
